...
# This happens in the middle of imports because some classes require the pygame display
# to be available before they can be imported.
pygame.init()
...
icon = ASSETS_FOLDER / "Icons" / "icon_small.png"
if icon.is_file():
    pygame.display.set_icon(pygame.image.load(icon))
else:
    handle_exception(f"File {FileNotFoundError(icon.resolve())} not found.")
...
def main(
    ...
    """Run the full game: menus, loading, the main play loop, and level transitions."""
    ...
    if meta_dict.get("MAIN_MENU") is not None and meta_dict["MAIN_MENU"].get("title_screen") is not None:
        title_screen_file = ASSETS_FOLDER / "TitleScreens" / meta_dict["MAIN_MENU"]["title_screen"]
    else:
        title_screen_file = ASSETS_FOLDER / "TitleScreens" / "title.png"

    if meta_dict.get("MAIN_MENU") is not None and meta_dict["MAIN_MENU"].get("title_screen_retro") is not None:
        title_screen_retro_file = ASSETS_FOLDER / "TitleScreens" / meta_dict["MAIN_MENU"]["title_screen_retro"]
    else:
    ...
    while True:
        ...
        if title_screen_file.is_file():
            ...
            if not controller.goto_main:
                for i in range(64):
                    ...
                    tm.sleep(0.01)
            controller.main_menu.buttons[1].is_enabled = (GAME_DATA_FOLDER / "save.p").is_file()
            ...
            handle_exception(f"File {FileNotFoundError(title_screen_file.resolve())} not found.")
        ...
        while True:
            ...
            if new_game:
                ...
                if load_data and cur_level == load_data["level"]:
                    should_load = True
            ...
            vfx_manager   = VisualEffectsManager(str(Path(ASSETS_FOLDER / "VisualEffects").resolve()))
            win.fill((0, 0, 0))
            if loading_screen:
                win.blit(loading_screen, ((win.get_width() - loading_screen.get_width()) / 2, (win.get_height() - loading_screen.get_height()) / 2))
            display_text("Loading mission... [3/3]", controller, min_pause_time=0, should_sleep=False, retro=controller.retro, background=True)
            controller.level = level = Level(cur_level or "", levels, meta_dict, objects_dict, {}, {}, player_audio or {}, enemy_audio or {}, block_audio or {}, message_audio or {}, vfx_manager, win, controller, loading_screen or pygame.Surface((0, 0), pygame.SRCALPHA))
            ...
            while True:
                ...
                if (controller.goto_load and (GAME_DATA_FOLDER / "save.p").is_file()) or controller.goto_main or controller.goto_restart or (controller.next_level is not None):
                    break
