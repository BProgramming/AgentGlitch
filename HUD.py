    ...
    def __init__(self, player, win, grayscale: bool=False):
        ...
        for i in range(10):
            ...
            if self.time_characters.get(c) is None:
                handle_exception(f'File {str(FileNotFoundError(join(ASSETS_FOLDER, "Icons", "Timer", f'{c}.png')))} not found.')
        if self.time_characters.get("COLON") is None:
            handle_exception(f'File {str(FileNotFoundError(join(ASSETS_FOLDER, "Icons", "Timer", "colon.png")))} not found.')
        if self.time_characters.get("DECIMAL") is None:
            handle_exception(f'File {str(FileNotFoundError(join(ASSETS_FOLDER, "Icons", "Timer", "decimal.png")))} not found.')
        ...
        if not isfile(file):
            handle_exception(f'File {str(FileNotFoundError(file))} not found.')
        else:
